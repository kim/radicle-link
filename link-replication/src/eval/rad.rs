    ...
    <C as Identities>::Urn: Clone + Debug,
{
    ...
    for urn in newest.delegate_urns() {
        ...
        let delegate = {
            let ids = format!("rad/ids/{}", urn_enc);
            let refname = refs::remote_tracking(remote, BString::from(ids));
            let head = Refdb::refname_to_id(cx, &refname)?
                .ok_or_else(|| format!("missing {}", refname.as_ref()))?;
