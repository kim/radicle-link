    ...
    Urn: ids::Urn,
{
    ...
    fn try_from(qualified: Qualified<'a>) -> Result<Self, Self::Error> {
        ...
        let mut iter = qualified.components();
        {
            let refs = iter.next().and_then(|c| c.as_lit());
            debug_assert_eq!(Some(lit::Refs), refs, "`Qualified` must start with 'refs/'");
        }
        ...
        let next = iter
            .next()
            .expect("qualified guarantees one component after refs/");

        if REMOTES == next.as_str() {
            let remote = Some(
                iter.next()
                    .ok_or(Error::Missing("remote peer"))
                    .and_then(|s| s.as_str().parse::<PeerId>().map_err(Error::from))?,
            );
            parse_inner(iter)
                .ok_or(Error::Other)
                .map(|inner| Parsed { remote, inner })
        } else {
            parse_inner(iter::once(next).chain(iter))
                .ok_or(Error::Other)
                .map(|inner| Parsed {
                    remote: None,
                    inner,
