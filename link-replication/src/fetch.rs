...
impl<T> Fetch<T> {
    fn scoped<'a, 'b: 'a>(&self, id: &'a PeerId, name: Qualified<'b>) -> refs::Scoped<'a, 'b> {
    ...
    fn ref_prefixes(&self) -> Vec<RefPrefix> {
        ...
        let signed = self
            ...
            .flat_map(move |(id, refs)| {
                refs.refs.iter().map(move |(name, _)| {
                    let name = Qualified::from_refstr(name).expect("sigrefs should be qualified");
                    RefPrefix::from(self.scoped(id, name))
                })
            });
