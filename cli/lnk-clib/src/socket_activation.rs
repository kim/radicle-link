...
///
/// Consequently, the [`Systemd`] method is provided for "unix" targets, while
/// [`Launchd`] is only available on "macos" targets.
pub trait Sockets {
    /// Obtain a managed socket by name.
    ///
    /// Note that `systemd` socket units allow only one file descriptor per name,
    /// while `launchd` services _may_ hand out more than one.
